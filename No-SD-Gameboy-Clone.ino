...
void setup() {
  ...
  //clear the TFT screen, add score
  TFTscreen.setTextSize(2);
  TFTscreen.background(0, 0, 0);
  TFTscreen.stroke(255, 255, 255);
  TFTscreen.text("Score:", 5, 5);
  TFTscreen.text("0", 80, 5);

  //random seed generated by reading empty port
  randomSeed(analogRead(A6)*654873/analogRead(A5));
}
...
void loop() {
  if(noCollision){  
    ...
    TFTscreen.text(str, 80, 5);
    
    byte line = eliminateLine();
    score = score + (line * line * 10);
    itoa(score, str, 10);
  ...
  //rotation
  if(y > 700){
    if((rotateHoldCount == 0 || (rotateHoldCount > 4 && rotateHoldCount % 2 == 0)) && validateDraw(0, 0, 1)){
...
bool validateDraw(int change_x, int change_y, int change_rotation){
  for(int z = 0; z < 4; z++){
    int x = pieces[shapeID][z][0];
    int y = pieces[shapeID][z][1];
  
    for(int a = 0; a < piece_rotation + change_rotation; a++){
      int hold = x * -1;
      x = y;
      y = hold;
    }
